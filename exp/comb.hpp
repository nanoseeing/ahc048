...
class ColorMixer {
  public:
    ...
    vector<Result> find_topN(const Color& t, int topN) const {
        ...
        sort(results.begin(), results.end(),
             [&](Result const& a, Result const& b) { return a.squared_error < b.squared_error; });
        return results;
    }
  ...
  private:
    ...
    // 事前準備：サイズ2〜4の全組み合わせを列挙し、Gram・擬似逆行列を計算
    void prepare_subsets() {
        // 組み合わせサイズを {2,3,4} に固定
        vector<int> sizes = {2, 3, 4};
        ...
        // これで subsets.size() は (K choose 2) + (K choose 3) + (K choose 4) になる
    }
};
