...
const int INIT_PARTITION_POS = 9; // input_data.N - 1 - 1
...
struct State {
    ...
    void apply_deliver(const Action &action) {
        ...
        if(this->paints[id].vol < 1.0 - 1e-6) {
            ...
            Output output = Output{this->init_wall, this->actions}; // tmp
            output.print_output();
            exit(1);
        };
...
double eval_cost(Input &input, State &state, vector<ImmediateInfo> &immeediate_info) {
    ...
    for(auto info : immeediate_info) {
        Paint paint = state.get_paint(0, info.k);
    ...
    double add_cost = input.D * max(0, state.add_cnt + add_cnt - (state.deliver_cnt + 1));

    return err_cost + discard_cost;
}

DicisionAction dicision_action(Input &input, State &state, ColorMixer &mixer, int obj_turn, vector<int> &partition_positions) {
...
void solve() {
    ...
    for(int i : range(input.H)) {
        ...
        if(i % 20 == 0) {
            ...
            cerr << boost::format("Turn: %d, V: %d, E: %d, deliver_cnt: %d, Score: %d (deliver: %d, err: %d)") % state.turn % state.add_cnt % state.error %
                        state.deliver_cnt % total_cost % deliver_cost % err_cost
                 << endl;
        }
