  ...
  public:
    ...
    tuple<vector<ImmediateInfo>, double> eval_one_result(ColorMixer::Result &result, State &state, Input &input) {
        ...
        for(int i : range(comb_size - 1)) {
            ...
            int it_ind = -1;
            int rate_item_size = (int)rate_item.rates[TEMP_MUL_CNT].size();
            bool is_add = weight > rate_item.init_vol;
            if(!is_add) {
                // addしない場合
                auto it = upper_bound(ALL(rate_item.vols[TEMP_MUL_CNT]), weight);
                it_ind = distance(rate_item.vols[TEMP_MUL_CNT].begin(), it);
            } else {
                // addする場合
                auto it = upper_bound(ALL(rate_item.post_add_vols[TEMP_MUL_CNT]), weight);
                it_ind = distance(rate_item.post_add_vols[TEMP_MUL_CNT].begin(), it);
            }
            ...
            for(int j : range(-1, 1)) {
                ...
                info.k = k;
                info.is_add = is_add;
                info.vol = is_add ? rate_item.post_add_vols[TEMP_MUL_CNT][new_ind] : rate_item.vols[TEMP_MUL_CNT][new_ind];
                info.fractors = rate_item.fractors[TEMP_MUL_CNT][new_ind];
                info.rate = rate_item.rates[TEMP_MUL_CNT][new_ind];
        ...
        cartesian_product(now_infos, [&](const vector<ImmediateInfo> &comb) {
            ...
            int it_ind = -1;
            if(!is_add) {
                auto it = lower_bound(ALL(last_item.vols[TEMP_MUL_CNT]), search_vol);
                it_ind = distance(last_item.vols[TEMP_MUL_CNT].begin(), it);
            } else {
                auto it = lower_bound(ALL(last_item.post_add_vols[TEMP_MUL_CNT]), search_vol);
                it_ind = distance(last_item.post_add_vols[TEMP_MUL_CNT].begin(), it);
            }

            if(it_ind >= (int)last_item.rates[TEMP_MUL_CNT].size()) {
                // 1g未満のペイントは無視
            } else {
                ImmediateInfo last_info;
                last_info.k = last_k;
                last_info.is_add = is_add;
                last_info.vol = is_add ? last_item.post_add_vols[TEMP_MUL_CNT][it_ind] : last_item.vols[TEMP_MUL_CNT][it_ind];
                last_info.fractors = last_item.fractors[TEMP_MUL_CNT][it_ind];
                last_info.rate = last_item.rates[TEMP_MUL_CNT][it_ind];
...
tuple<DicisionAction, PrimitiveColorGroupInfo> construct_from_immediateinfo(vector<ImmediateInfo> &best_info, PrimitiveColorGroupInfo &primitive_group_info) {
    ...
    for(auto &info : best_info) {
        ...
        // 分割1回適応
        if(frac_size == 1) {
            auto &fractor = info.fractors[0];
            int diff = fractor.second - fractor.first;
            ...
            auto denominator_act = Action::Toggle(fractor.second - 1, info.k, fractor.second, info.k);
            auto numerator_act = Action::Toggle(diff - 1, info.k, diff, info.k);
            if(fractor.first == 1 && fractor.second == 1) {
                // 全開放
                ...
                if(fractor.second == now_partition_pos + 1) {
                    // 現在の仕切りを動かさない場合
                ...
                if(diff == 1) {
                    ...
                    new_group_info.partition_positions[info.k] = diff - 1;
                }
            }
        } else {
            // 分割n回適応
            ...
            for(int fi : range(frac_size)) {
                ...
                // cpp_dump(fractor, upper_partition, lower_partition, stop_par_pos, release_par_pos);
