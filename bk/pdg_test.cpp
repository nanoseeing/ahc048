...
//   もし収束しなければ最後の反復値を返す。
// -----------------------------------------------------------------------------
Eigen::VectorXd solveConvexComboByPGD(const Eigen::Matrix<double, 3, Eigen::Dynamic>& A, const Eigen::Vector3d& t, int maxIter = 1000, double alpha = 1e-2,
                                      double tol = 1e-8) {
...
int main() {
    ...
    // --------- (B) 目標色 t を用意 ------------
    // たとえば、#0,#2,#4 を 0.2:0.5:0.3 の割合で混ぜた色
    auto t_color = input.target[0];
    Eigen::Vector3d t = Eigen::Vector3d(t_color[0], t_color[1], t_color[2]);

    // --------- (C) 部分集合サイズ k を指定 ------------
    const int k_subset = 3; // 例えば「3 本から選ぶ」ケース
